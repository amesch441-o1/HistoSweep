...
ratio_norm_image, mask1, mask1_updated, mask2, output_dir, super_pixel_size=16, generate_masked_plots = False):
    ...
    # Ensure `prefix` is a folder path, not a file
    if os.path.isfile(prefix):
        prefix = os.path.dirname(prefix)


    # Step 1: Create directories (prefix should not be added again)
    output_dir_plots = os.path.join(output_dir, "AdditionalPlots")
    ...
    # Step 2: Load masks (prefix should not be prepended again)
    mask_path = os.path.join(output_dir, "mask.png")
    mask_small_path = os.path.join(output_dir, "mask-small.png")
